 ...
 * Uses window.TimrX and window.State globals (no ES modules).
 */

(function() {
  'use strict';

  const { byId, dateLabel, normalizeEpochMs, HISTORY_MENU_EDGE_PAD, HISTORY_SUBMENU_GAP } = window.TimrX;
  ...
  function getActiveHistoryMenu() {
    return { btn: activeHistoryMenuBtn, menu: activeHistoryMenu };
  }

  function getActiveHistorySubmenu() {
  ...
  function shortTitle(input, words = 6) {
    let t = '';
  ...
  function compareHistoryModels(a = {}, b = {}) {
    ...
    if (timeA !== timeB) {
      return State.historyState?.sort === 'asc' ? timeA - timeB : timeB - timeA;
    }
  ...
  function closeActiveHistorySubmenu() {
  ...
  function closeActiveHistoryMenu() {
  ...
  function openHistoryMenu(menuBtn, menu) {
  ...
  function openHistorySubmenu(submenuBtn, submenu) {
  ...
  function updateActiveHistoryMenuPosition() {
  ...
  // ============================================================================
  // HTML TEMPLATES (abbreviated for brevity - keeping full functionality)
  // ============================================================================
  ...
  function buildHistoryThumb(bundle = {}, isExpanded = false) {
    const State = getState();
    const historyActiveModelId = State.historyActiveModelId;
    const historyFreshThumbs = State.historyFreshThumbs || new Set();
  ...
  function getFilteredHistory() {
    const State = getState();
    const historyState = State.historyState || { query: '', filter: 'all' };
    const q = (historyState.query || '').toLowerCase();
    const filter = historyState.filter || 'all';
    const raw = State.getHistory ? State.getHistory() : [];
  ...
  function renderHistory() {
    const State = getState();
    const historyState = State.historyState || { page: 1, pageSize: 9, galleryExpanded: false, filter: 'all', sort: 'desc' };
    const historyLineageCounts = State.historyLineageCounts || new Map();
    const getActiveJobs = State.getActiveJobs || (() => []);
    ...
    if (historyState.filter === 'image') {
      ...
      let slice = isGallery ? sortedImages : sortedImages.slice(start, Math.min(start + pageSize, totalImages));

      const imageGridMarkup = slice.map(img => buildHistoryThumb({ models: [img], isBundle: false }, false)).join('');
      grid.innerHTML = `<div class="history-image-grid">${imageGridMarkup}</div>`;

      if (pageLabel) {
        pageLabel.textContent = isGallery ? `Gallery - ${totalImages} images` : `${historyState.page}/${pages}`;
      }

      const disableNav = (btn, shouldDisable) => btn && (shouldDisable ? btn.setAttribute('disabled', '') : btn.removeAttribute('disabled'));
    ...
    const skeletonMarkup = isLoading && !sortedLineages.length ? buildHistorySkeleton(isGallery ? 1 : 2, isGallery ? 5 : 4) : '';
    ...
    const totalAssets = sortedLineages.reduce((sum, l) => sum + (l.models?.length || 0), 0);
    ...
    let start = (historyState.page - 1) * pageSize;
    let slice = isGallery ? sortedLineages : sortedLineages.slice(start, Math.min(start + pageSize, totalRows));

    const timelineMarkup = slice.map(lineage => {
      const rowKey = String(lineage.rootId || lineage.id);
      const previousCount = historyLineageCounts.get(rowKey) || lineage.models.length;
      ...
      const countElement = hasMore
        ? `<button class="history-collection__count" type="button" data-action="toggle-collection" data-lineage-key="${rowKey}" aria-expanded="false">
            All ${lineage.models.length} assets <span class="history-collection__arrow">&#8250;</span>
            ${delta > 0 ? `<span class="history-collection__counter">+${delta}</span>` : ''}
          </button>`
        : `<span class="history-collection__count">All ${lineage.models.length} assets &#8250;</span>`;
      ...
      return `
        <div class="history-collection" data-lineage-root="${rowKey}">
          ...
          <div class="history-collection__thumbs">${visibleThumbsMarkup}</div>
    ...
    if (pageLabel) {
      pageLabel.textContent = isGallery ? `Gallery - ${totalAssets} assets` : `${historyState.page}/${pages}`;
    }
  ...
  // ============================================================================
  // EXPOSE GLOBALLY via window.History
  // ============================================================================
  window.History = {
    shortTitle,
