/**
 * main.js
 * The entry point. Runs initialization logic and sets up the primary event listeners.
...
(function() {
  ...
  const { byId, safe, log, onThreeReady, normalizeEpochMs, apiFetch } = window.TimrX;
  ...
  function switchHistoryFilter(filter = 'all') {
    ...
    if (State.historyState && State.historyState.filter !== filter) {
  ...
  function handleImageFile(file) {
    ...
    if (UI.state) UI.state.imageFile = file;
    const url = URL.createObjectURL(file);
    if (imagePreview) {
      imagePreview.src = url;
      imagePreview.classList.remove('hidden');
    }
    if (UI.updateGenerateHint) UI.updateGenerateHint();
  }
  ...
  function handleModelFile(file) {
    ...
    if (UI.state) UI.state.modelFile = file;
  ...
  function migrateHistoryDates() {
    const State = getState();
    if (!State.getHistory || !State.saveHistory) return;
  ...
  function setupGenerateButtonListeners() {
    ...
    leftStack.addEventListener('click', (e) => {
      ...
      if (btnId === 'applyRemeshBtn' && API.startRemeshFromPanel) {
        API.startRemeshFromPanel();
        return;
      }
      if (btnId === 'generateTextureBtn' && API.startTextureFromPanel) {
        API.startTextureFromPanel();
        return;
      }
      if (btnId === 'applyRigBtn' && API.startRigFromPanel) {
        API.startRigFromPanel();
        return;
      }
      if (!btnId || !btnId.includes('generate')) return;

      if (btnId === 'generateModelBtn' && API.onGenerateClick) {
        API.onGenerateClick();
      } else if (btnId === 'generateImageBtn' && API.startImageGenerationByProvider) {
        API.startImageGenerationByProvider();
      } else if (btnId === 'generateVideoBtn' && API.onGenerateClick) {
        API.onGenerateClick();
      }
    });
  ...
  function initViewerToolbar() {
    ...
    toolbar.addEventListener('click', (e) => {
      ...
      const activeItem = API.getActiveHistoryItem ? API.getActiveHistoryItem() : null;
      ...
      if (action === 'texture' && activeItem && API.startTextureFromHistory) {
        API.startTextureFromHistory(activeItem);
      }

      if (action === 'remesh' && activeItem && API.startRemeshFromHistory) {
        API.startRemeshFromHistory(activeItem);
      }

      if (action === 'rig' && activeItem && API.startRigFromHistory) {
  ...
  function initUi() {
    ...
    if (UI.initTabRefs) UI.initTabRefs();
    ...
    // Set initial tab
    if (UI.setActiveTab) UI.setActiveTab('text3d');
      ...
      if (textarea) {
        textarea.addEventListener('keydown', (e) => {
          if (e.key === 'Enter' && !e.shiftKey && !e.repeat) {
            ...
            if (genBtn && API.onGenerateClick) API.onGenerateClick();
          }
        });
      }
    });
    ...
    if (UI.initNiceSelects) UI.initNiceSelects();
    if (UI.updateGenerateHint) UI.updateGenerateHint();
  }
  ...
  function wireGallery() {
    ...
    const renderHistory = History.renderHistory || window.renderHistory || function() {};
    const shortTitle = History.shortTitle || window.shortTitle || function(x) { return x?.prompt?.slice(0,50) || '(untitled)'; };
    const closeActiveHistoryMenu = History.closeActiveHistoryMenu || function() {};
    const closeActiveHistorySubmenu = History.closeActiveHistorySubmenu || function() {};
    const openHistoryMenu = History.openHistoryMenu || function() {};
    const openHistorySubmenu = History.openHistorySubmenu || function() {};
    const getFilteredHistory = History.getFilteredHistory || function() { return State.getHistory ? State.getHistory() : []; };
    const getActiveHistoryMenu = History.getActiveHistoryMenu || function() { return {}; };
    const getActiveHistorySubmenu = History.getActiveHistorySubmenu || function() { return {}; };
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', async () => {
        ...
        try {
          if (State.forceRestoreFromDB) await State.forceRestoreFromDB();
    ...
    if (next) next.addEventListener('click', () => {
      ...
      const pageSize = State.historyState?.pageSize || 9;
      const total = Math.max(1, Math.ceil(filtered.length / pageSize));
      if (State.historyState && State.historyState.page < total) {
        State.historyState.page++;
        renderHistory();
      }
    });
    ...
    if (grid) {
      grid.addEventListener('click', async (e) => {
        ...
        const item = historyArr.find(x => x.id === id);
        if (!item) return;
        ...
        if (act === 'open') {
          ...
          if (wasGallery && State.historyState) {
            State.historyState.galleryExpanded = false;
            State.historyState.page = 1;
          }

          if (!glbUrl && (item.type === 'image' || item.image_url)) {
            if (State.setHistoryActiveModelId) State.setHistoryActiveModelId(id);
            renderHistory();
            ...
            if (imgSrc) {
              ...
              if (Viewer.showImageInViewer) Viewer.showImageInViewer(imgSrc);
            }
            return;
          }
          ...
          if (Viewer.loadModelWithFallback) await Viewer.loadModelWithFallback(primary, fallback);
        ...
        if (act === 'texture' && API.startTextureFromHistory) {
          await API.startTextureFromHistory(item);
          return;
        }

        if (act === 'remesh' && API.startRemeshFromHistory) {
          await API.startRemeshFromHistory(item);
          return;
        }

        if (act === 'rig' && API.startRigFromHistory) {
          await API.startRigFromHistory(item);
          return;
        }

        if (act === 'refine' && API.onPostProcessFromHistory) {
          API.onPostProcessFromHistory(item, 'refine');
          return;
        }

        if (act === 'image-to-3d' && API.startImageTo3DFromHistory) {
        ...
        if (act === 'delete') {
          ...
          try {
            ...
            if (State.deleteHistoryItem) State.deleteHistoryItem(id, { skipRemote: true });
            renderHistory();
          } catch (err) {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      if (window.Converter?.init) window.Converter.init();
    } catch (e) {
    ...
    onThreeReady(async () => {
      ...
      const creditsPromise = (async () => {
        if (window.WorkspaceCredits?.init) {
          try {
            await window.WorkspaceCredits.init();
          } catch (e) {
      ...
      // Initialize viewer
      try {
        if (Viewer.initViewer) Viewer.initViewer();
      } catch (e) {
      ...
      try {
        if (State.loadHistoryFromDB) await State.loadHistoryFromDB();
        renderHistory();
      } catch (e) {
      ...
      if (UI.showOutputEmpty) UI.showOutputEmpty();

      // Ensure credits are loaded before resuming jobs
      await creditsPromise;

      // Resume any pending jobs
      if (API.resumePendingJobs) await API.resumePendingJobs({ skipEmptyUI: true });
