...
(function () {
  'use strict';
  var TEAL = [127, 200, 194], SAND = [184, 167, 122], GREEN = [168, 200, 127], RED = [217, 138, 138];
  ...
  function drawPrintPrep(ctx, w, h, t, card) {
    ...
    faces.forEach(function (f) {
      ...
      if (printed || done) {
        ...
        ctx.fillStyle = rgba(TEAL, 0.10 + 0.16 * lum + glow);
        ctx.fill();
        ...
        ctx.strokeStyle = bad && Math.sin(t * 9 + f.i) > 0.2
          ? rgba(RED, 0.7) : 'rgba(255,255,255,0.10)';
        ctx.lineWidth = 1 * devicePixelRatio;
        ctx.stroke();
      }
    });
    ...
    if (!done) {
      ...
      grad.addColorStop(0, 'rgba(127,200,194,0)');
      grad.addColorStop(0.5, rgba(repairing ? GREEN : TEAL, repairing ? 0.95 : 0.8));
      grad.addColorStop(1, 'rgba(127,200,194,0)');
      ctx.fillStyle = grad;
      ctx.fillRect(w * 0.12, ly - 1.2 * devicePixelRatio, w * 0.76, 2.4 * devicePixelRatio);
      /* sparks on the plane */
      for (var s = 0; s < 7; s++) {
        var sx = w * (0.25 + 0.5 * prand(s + Math.floor(t * 6)));
        ctx.fillStyle = rgba(repairing ? GREEN : TEAL, 0.7 * prand(s * 3 + Math.floor(t * 10)));
  ...
  function drawRemesh(ctx, w, h, t, card) {
    ...
    faces.forEach(function (f) {
      ...
      if (painted) {
        ...
        ctx.fillStyle = 'rgba(' +
          Math.round(lerp(TEAL[0], SAND[0], warm * 0.45)) + ',' +
          Math.round(lerp(TEAL[1], SAND[1], warm * 0.45)) + ',' +
          Math.round(lerp(TEAL[2], SAND[2], warm * 0.45)) + ',' + (0.14 + 0.30 * lum) + ')';
        ctx.fill();
      }
      var lineA = snap > 0.5
        ? rgba(TEAL, 0.20 + 0.35 * snap * (1 - dissolve))
        : 'rgba(255,255,255,' + (0.10 + 0.10 * Math.abs(Math.sin(f.i + t * 2))) + ')';
    ...
    if (paint > 0 && paint < 1) {
      ...
      grad.addColorStop(0, 'rgba(184,167,122,0)');
      grad.addColorStop(1, rgba(SAND, 0.6));
      ctx.fillStyle = grad;
  ...
  function drawExport(ctx, w, h, t, card) {
    ...
    halo.addColorStop(0, rgba(TEAL, 0.10 + pulse * 0.25));
    halo.addColorStop(1, 'rgba(127,200,194,0)');
    ctx.fillStyle = halo;
    ctx.fillRect(cx - scale * 3, cy - scale * 3, scale * 6, scale * 6);
    CUBE_E.forEach(function (e) {
      ctx.strokeStyle = rgba(TEAL, 0.35 + 0.45 * Math.min(cp[e[0]][2], cp[e[1]][2]) + pulse * 0.3);
    ...
    FORMATS.forEach(function (label, i) {
      ...
      if (lt < 2.0) {                                  /* travelling in */
        ...
        for (var k = 1; k <= 5; k++) {
          ...
          ctx.fillStyle = rgba(SAND, 0.25 * (1 - k / 6));
          ctx.beginPath(); ctx.arc(tp[0], tp[1], (3 - k * 0.4) * dpr, 0, 7); ctx.fill();
        }
        chipDraw(ctx, pos[0], pos[1], label, SAND, Math.min(1, tt * 4) * (1 - Math.max(0, tt - 0.92) * 12), dpr);
      } else if (lt < 2.35) {
        ...
        for (var k2 = 1; k2 <= 6; k2++) {
          ...
          ctx.fillStyle = rgba(TEAL, 0.3 * (1 - k2 / 7));
          ctx.beginPath(); ctx.arc(tp2[0], tp2[1], (3.4 - k2 * 0.4) * dpr, 0, 7); ctx.fill();
        }
        chipDraw(ctx, opos[0], opos[1], OUT, TEAL, 1 - Math.max(0, ot - 0.8) * 5, dpr);
        latest = label;
      }
    });
