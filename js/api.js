...
function parseInsufficientCreditsError(errorMsg) {
  ...
  // Unified format (Sep 2026): one wallet, one error string
  const unifiedMatch = errorMsg.match(/INSUFFICIENT_CREDITS:required=(\d+):balance=(\d+):reserved=(\d+):available=(\d+)/);
  if (unifiedMatch) {
    return {
      creditType: 'general',
      required: parseInt(unifiedMatch[1], 10),
      balance: parseInt(unifiedMatch[2], 10),
      reserved: parseInt(unifiedMatch[3], 10),
      available: parseInt(unifiedMatch[4], 10),
    };
  }
